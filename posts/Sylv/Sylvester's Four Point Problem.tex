...
\title{%
 \huge Sylvester's Four Point Problem \\ 
  \Large Introduction to Crofton's Formula  \\
...
\end{Prob}
\begin{figure}[h]
\includegraphics[width=8cm]{retrant}
\centering
\caption{Retrant Quadrilateral}
\end{figure}
...
In this paper, I investigate the problem when $K$ is a disk of radius $r$, $D_r$. At first, I will attempt the problem with techniques from class and, hopefully, demonstrate that we need tools outside traditional probability theory to solve the problem reasonably. 
 ...
 \section*{Calculation: Brute Force}
 
 
Let $D_r$ be a disk of radius $r>0$. Sample four random points from the disk $D_r$:  $$M,N,P,Q \sim \textnormal{Unif}(D_r)$$ 
...
We can define the joint probability density function to be:  \[ f_{M,N,P}(m,n,p) = \begin{cases}
...
%\[ d x_i = -r \sin \theta_i ~d\theta_i  \textnormal { and } d y_i = r \cos \theta_i ~d\theta_i \]
...
We can further specify what the joint pdf looks like:   \begin{align*}
...
Consider a new domain $D_{r+h}$, a disk of radius $r+h$. Like before, sample points from the domain:  \[ \vec{Q_1}, \vec{Q_2}  \sim  \textnormal{Unif}(D_{r+h}) \] One can ask what is the relationship between the expected areas of points sampled from the domains: $D_r$ and $D_{r+h}$. 

\begin{figure}[h]
\includegraphics[width=8cm]{concentric}
\centering
\end{figure}


We first compute the joint probability of the points in the bigger domain, $D_{r+h}$:  \[ P_{r+h}(\vec{Q_1}, \vec{Q_2})  = \begin{cases}
...
Now consider the case when $\vec{Q_1}, \vec{Q_2} \in D_r$:   \begin{align*}
...
We can similarly compute the probability one of the points is contained inside $D_r$ and the other in $D_{r+h}- D_r$:  \begin{align*}
...
Lastly, compute the probability that both the points are in $D_{r+h}-D_r$:  \begin{align*}
...
Denote $P_{r+h} = P$. Then, using conditional expectation, one can show that  \begin{align*}
...
\end{align*}
Taking the limit as $h \to 0$, we get:  \[ \mathbb{E}'[D_r] = \frac{4}{r} ~\mathbb{E}[ D_r, \left(D_{r} - D_{r}\right)] - \frac{4}{r}~ \mathbb{E}[  D_{r}] 
 \]
We need to be careful about taking the limits on the right hand side. Consider the first term of the summand: $\mathbb{E}[  \text{ length } | ~   \vec{Q_i} \in  D_r, \vec{Q_j} \in D_{r+h} - D_{r}]$. As $h \to 0$, the domain $D_{r+h} - D_r$ collapses to $C_r$, the boundary of the disk $D_r$. As a result:  \[ \lim_{h \to 0}  \mathbb{E}[  \text{ length } | ~   \vec{Q_i} \in  D_r, \vec{Q_j} \in D_{r+h} - D_{r}] = \mathbb{E}[  \text{ length } | ~   \vec{Q_i} \in  D_r, \vec{Q_j} \in C_r]\] 
...
\[|\vec{Q_1'}- \vec{Q_2'}| =r |\vec{Q_1}- \vec{Q_2}| =kr  \] Since expectation is linear, we know  \[ \E[|\vec{Q_1'}- \vec{Q_2'}|] = \E[k|\vec{Q_1}- \vec{Q_2}|] = k \E[|\vec{Q_1}- \vec{Q_2}| \] 

I mention this because this tell us that rate of change of expected length should be a constant:  \[ \mathbb{E}'[ \textnormal{ length } | D_{r}]= k  \] 
...
We have one remaining expected to compute which is relatively simpler:  \begin{align*}
\mathbb{E}[ |\vec{Q_1}- \vec{Q_2}| \textnormal{ given } \vec{Q_1} \in  D_r, \vec{Q_2} \in C_r] 
\end{align*} 

Note that due to the symmetry of the circumference, we can fix $\vec{Q_2}$ at the origin: $\vec{Q_2} = (0,0)$. \begin{figure}[h]
\includegraphics[width=15cm]{integration}
\centering
\end{figure}Then:  \begin{align*}
\mathbb{E}[ |\vec{Q_1}- \vec{Q_2}| \textnormal{ given } \vec{Q_1} \in  D_r, \vec{Q_2} =(0,0)] &= \int_{D_r} \sqrt{x_2^2+ y_2^2} ~f_{\vec{Q_1}}
\end{align*}
Here $f_{\vec{Q_1}}$ is simply the probability distribution function of $\vec{Q_1}$ which we know equals the inverse of the area of $D_r$. \begin{align*}
\mathbb{E}[ |\vec{Q_1}- \vec{Q_2}| \textnormal{ given } \vec{Q_1} \in  D_r, \vec{Q_2} =(0,0)] &= \frac{1}{\Area(D_r)} \int_{D_r} \sqrt{x_2^2+ y_2^2} 
\end{align*}
We can parametrize $\vec{Q_2}$ as a function of $\mu$ and $\theta$ where $\mu =  \sqrt{x_2^2+ y_2^2} $ where $\theta = [-\frac{\pi}{2}, \frac{\pi}{2}]$. From the picture above, we see that as $\mu \in [0, 2R\cos(\theta)]$. As a result, we get:  \begin{align*}
...
Recall the remark we made on the last page: 
 ...
 Setting $r=1$, we get: 
  ...
  The nice thing about Crofton's formula is that it generalizes for any convex set $K$, and $n$ uniformly, randomly chosen points from $K$. We can then use a similar approach to find the expected area of a triangle whose vertices are sampled uniformly, randomly from the unit disk: \begin{align*}
  \E[\Area(\Delta)\textnormal{ given that } \Delta \in D_1]  &= \frac{35}{48\pi}
  \end{align*} The computation becomes slightly involved so I will leave it as an exercise for the reader. As a result, we know that the probability of the event where the convex hull of four randomly distributed points on a unit disk is a triangle, denoted by $E$ is:  \[ P(E) = \frac{4\E[\Area(\Delta)]}{\pi} = \frac{35}{12\pi^2} \] I like the result because its considered to be one of the earliest works in the world of geometric probability. Nice extensions to the problem can be found in the references below.
 
  
\begin{thebibliography}{9}

\bibitem{Eisen} Eisenberg, B., \& Sullivan, R. (2000). Crofton’s Differential Equation. The American Mathematical Monthly, 107(2), 129–139. https://doi.org/10.2307/2589434

\bibitem{Pfiefer}Pfiefer, R. E. (1989). The Historical Development of J. J. Sylvester’s Four Point Problem. Mathematics Magazine, 62(5), 309–317. https://doi.org/10.2307/2689482

\bibitem{Solomon} Solomon H. (1978). Geometric Probability / Herbert Solomon. Society for Industrial and Applied Mathematics; 1978.
